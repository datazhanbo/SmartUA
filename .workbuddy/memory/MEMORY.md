...
- **Phase A 真实数据地基 已完成（2026-07-11，v1.8.0）**：A1 双轨持久化（内存缓存+SQLite WAL，会话/记忆/告警重启不丢）；A2 GoogleAdsConnector 重写=**真实 google-ads SDK 实现 + Mock 回退**（凭证齐全且运行环境装了 google-ads→走真实 API，否则自动 Mock，含"凭证齐但 SDK 缺"也回退 Mock 防崩）；TikTokConnector 实现+注册（缺 token 自动 Mock）；Meta 解封改 `config.agent_default_platform="meta"` 即可；A3 BaseConnector.current_summary/account_status/simulate_impact 通用接地，缺 MMP 时 roi=None 安全。验证：scripts/smoke_phaseA.py 13/13 PASS。详见 docs/AGENT_ITERATION_ROADMAP.md。
